...
% Plot performance data for JI_8x8Kernel
if ( 1 )
  system( 'cp ../../../Week2/C/data/output_JI_8x8Kernel.m .' );
  output_JI_8x8Kernel   % load data
  assert( max(abs(data(:,6))) < 1.0e-10, ...
      'Hmmm, better check if there is an accuracy problem');
...
% Plot time data for IJP_JI_8x8Kernel (to plot change "0" to "1")
if ( 1 )
  output_IJP_JI_8x8Kernel
  assert( max(abs(data(:,6))) < 1.0e-10, ...
      'Hmmm, better check if there is an accuracy problem');
  plot( data(:,1), data(:,5), 'DisplayName', 'IJP\_JI\_8x8Kernel', 'MarkerSize', 8, 'LineWidth', 2, ...
        'Marker', 'x', 'LineStyle', '-.', 'Color', plot_colors( 2,: ) );
end
...
set( legend2, 'Location', 'east', 'FontSize', 14) ;
